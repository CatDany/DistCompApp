...
public class SyracuseSolver implements Runnable
{
	...
	private void reportDone()
	{
		...
		com.sendToServer(String.format("SPDONE %s %s", now - startTime, initialStr));
		if (ProgressActivity.activityInstance != null && !ProgressActivity.activityInstance.isPaused) {
			ProgressActivity.activityInstance.setNumberInProgress(initialStr);
			BFLog.d("LOL XDDDDDDDD");
		}
	}
	...
	{
		if (!interrupted || !current.equals(bigOne))
		{
			long now = System.currentTimeMillis();
